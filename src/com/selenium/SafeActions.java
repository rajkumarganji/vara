...
public class SafeActions extends Sync {
	...
	public void safeClick(String locato, int... optionWaitTime) {
		int waitTime = 0;
		try {
			waitTime = getWaitTime(optionWaitTime);
			...
			setHighlight(element);
			element.click();
			log.info("Clicked on the element " + locator1);
		}
	}
	 ...
	 * @param waitTime
	 */
	public void safeJavaScriptDblClick(By locator, int... optionWaitTime) {
		int waitTime = 0;
		try {
			waitTime = getWaitTime(optionWaitTime);
			if (waitUntilClickable(locator, waitTime)) {
				...
				((JavascriptExecutor) driver).executeScript(jsDoubleClick, element, 0, 0);
	...
	public void safeClearAndType(By locator, String text, int... optionWaitTime) {
		...
		try {
			...
			if (isElementPresent(locator, waitTime)) {
				...
				log.error("Text " + text + " to be entered after " + " clear the field with " + locator
						+ " was not found in DOM in time - " + waitTime);
				Assert.fail("Text " + text + " to be entered after " + " clear the field with " + locator
						+ " was not found in DOM in time - " + waitTime);
			}
		} catch (StaleElementReferenceException e) {
			log.error("Text " + text + " to be entered after " + " clear the field with " + locator
			...
			log.error("Text " + text + " to be entered after " + " clear the field with " + locator
					+ " was not found in DOM in time - " + waitTime + " - NoSuchElementException");
			Assert.fail("Text " + text + " to be entered after " + " clear the field with " + locator
					+ " was not found in DOM in time - " + waitTime + " - NoSuchElementException");
		}
	...
	public void safeType(String locato, String text, int... optionWaitTime) {
		int waitTime = 0;
		try {
			waitTime = getWaitTime(optionWaitTime);
			By locator = By.xpath(locato);
	...
	public void safeSelectCheckboxes(int waitTime, By... locator) throws Exception {
		...
		try {
			...
			log.error("Element with locator- " + check
					+ " is not attached to the page document - StaleElementReferenceException");
			Assert.fail("Element with locator- " + check
					+ " is not attached to the page document - StaleElementReferenceException");
		} catch (NoSuchElementException e) {
	...
	public void safeDeselectCheckboxes(int waitTime, By... locator) {
		...
		try {
			...
			log.error(
					"Element with " + check + " is not attached to the page document - StaleElementReferenceException");
			Assert.fail(
					"Element with " + check + " is not attached to the page document - StaleElementReferenceException");
		} catch (NoSuchElementException e) {
			log.error("Element " + check + " was not found in DOM in time - " + waitTime + " - NoSuchElementException");
			Assert.fail(
					"Element " + check + " was not found in DOM in time - " + waitTime + " - NoSuchElementException");
		} catch (Exception e) {
	...
	public void safeSelectOptionInDropDown(By locator, String optionToSelect, int... optionWaitTime) {
		try {
			...
			if (isElementPresent(locator, waitTime)) {
				...
				Select select = new Select(selectElement); // Get a list of the options options =
				select.getOptions(); // For each option in the list, verify if it's the one you want and then click
										// it
				if (!options.isEmpty()) {
					for (WebElement option : options) {
						if (option.getText().contains(optionToSelect)) {
							safeJavaScriptClick(option, LONGWAIT);
	...
	/**
	 * Method - Safe Method for User get available options from Drop down, waits
	 * until the element is loaded and then selects an option from drop down
	 * 
	 * @param locator
	 * @param waitTime
	 * @return - List<WebElements> (returns List of WebElements -- Size equal to
	 *         options count)
	 * @throws Exception
	 */
	...
	public List<WebElement> safeGetOptionsInDropDown(By locator, int... optionWaitTime) {
		...
		try {
			...
			log.error("Dropdown element with " + locator
					+ " is not attached to the page document - StaleElementReferenceException");
			Assert.fail("Dropdown element with " + locator
					+ " is not attached to the page document - StaleElementReferenceException");
		} catch (NoSuchElementException e) {
			log.error("Dropdown element " + locator + " was not found in DOM in time - " + waitTime
					+ " - NoSuchElementException");
	...
	public void safeSelectOptionInDropDownByIndexValue(By locator, int iIndexofOptionToSelect, int... optionWaitTime) {
		...
		try {
			...
			if (isElementPresent(locator, waitTime)) {
				...
				log.info("Selected dropdown item by index option:" + iIndexofOptionToSelect + " from " + locator
						+ " dropdown");
			} else {
				log.error("Unable to select dropdown item by index option:" + iIndexofOptionToSelect + " from "
						+ locator);
				Assert.fail("Unable to select dropdown item by index option:" + iIndexofOptionToSelect + " from "
						+ locator);
			}
		} catch (StaleElementReferenceException e) {
			log.error("Dropdown item by index option:" + iIndexofOptionToSelect + " with " + locator
					+ " is not attached to the page document - StaleElementReferenceException");
			Assert.fail("Dropdown item by index option:" + iIndexofOptionToSelect + " with " + locator
					+ " is not attached to the page document - StaleElementReferenceException");
		} catch (NoSuchElementException e) {
			log.error("Dropdown item by index option:" + iIndexofOptionToSelect + " with " + locator
					+ " is not found in DOM in time - " + waitTime + "- NoSuchElementException");
			Assert.fail("Dropdown item by index option:" + iIndexofOptionToSelect + " with " + locator
					+ " is not found in DOM in time - " + waitTime + "- NoSuchElementException");
		} catch (Exception e) {
	...
	public void safeSelectOptionInDropDownByValue(By locator, String sValuefOptionToSelect, int... optionWaitTime) {
		...
		try {
			...
			if (isElementPresent(locator, waitTime)) {
				...
				log.info("Selected dropdown item by value option:" + sValuefOptionToSelect + " from " + locator
						+ " dropdown");
			} else {
				log.error(
						"Unable to select dropdown item by value option:" + sValuefOptionToSelect + " from " + locator);
				Assert.fail(
						"Unable to select dropdown item by value option:" + sValuefOptionToSelect + " from " + locator);
			}
		} catch (StaleElementReferenceException e) {
			log.error("Dropdown item by value option:" + sValuefOptionToSelect + " with " + locator
					+ " is not attached to the page document - StaleElementReferenceException");
			Assert.fail("Dropdown item by value option:" + sValuefOptionToSelect + " with " + locator
					+ " is not attached to the page document - StaleElementReferenceException");
		} catch (NoSuchElementException e) {
			log.error("Dropdown item by value option:" + sValuefOptionToSelect + " with " + locator
					+ " was not found in DOM in time - " + waitTime + " - NoSuchElementException");
	...
	public void safeSelectOptionInDropDownByVisibleText(By locator, String sVisibleTextOptionToSelect,
		...
		try {
			...
			if (isElementPresent(locator, waitTime)) {
				...
				log.info("Selected dropdown item by visible text option:" + sVisibleTextOptionToSelect + " from "
						+ locator + " dropdown");
			} else {
				log.error("Unable to select dropdown item by visible text option:" + sVisibleTextOptionToSelect
						+ " from " + locator);
				Assert.fail("Unable to select dropdown item by visible text option:" + sVisibleTextOptionToSelect
						+ " from " + locator);
			}
		} catch (StaleElementReferenceException e) {
			log.error("Dropdown item by visible text option:" + sVisibleTextOptionToSelect + " with " + locator
					+ " is not attached to the page document - StaleElementReferenceException");
			Assert.fail("Dropdown item by visible text option:" + sVisibleTextOptionToSelect + " with " + locator
					+ " is not attached to the page document - StaleElementReferenceException");
		} catch (NoSuchElementException e) {
			log.error("Dropdown item by visible text option:" + sVisibleTextOptionToSelect + " with " + locator
					+ " was not found in DOM - NoSuchElementException");
			Assert.fail("Dropdown item by visible text option:" + sVisibleTextOptionToSelect + " with " + locator
					+ " was not found in DOM - NoSuchElementException");
		} catch (Exception e) {
			log.error("Unable to select dropdown item by visible text option:" + sVisibleTextOptionToSelect + " from "
					+ locator + " dropdown" + " - " + e);
			Assert.fail("Unable to select dropdown item by visible text option:" + sVisibleTextOptionToSelect + " from "
					+ locator + " dropdown");
		}
	}
	...
	public void safeSelectListBox(By locator, String sOptionToSelect, int... optionWaitTime) {
		...
		try {
			...
			log.error("Listbox item: " + sOptionToSelect + " with " + locator
					+ " is not attached to the page document - StaleElementReferenceException");
			Assert.fail("Listbox item: " + sOptionToSelect + " with " + locator
					+ " is not attached to the page document - StaleElementReferenceException");
		} catch (NoSuchElementException e) {
			log.error("Listbox item: " + sOptionToSelect + " with " + locator + " was not found in DOM in time - "
					+ waitTime + " - NoSuchElementException");
			Assert.fail("Listbox item: " + sOptionToSelect + " with " + locator + " was not found in DOM in time - "
					+ waitTime + " - NoSuchElementException");
		} catch (Exception e) {
	...
	public void mouseHoverAndSelectOption(By locator, By byOptionlocator, int waitTime) {
		try {
			...
			log.error("Unable to hover the cursor on " + locator + "or unable to select " + byOptionlocator + "option"
					+ " - " + e);
			Assert.fail(
					"Unable to hover the cursor on " + locator + "or unable to select " + byOptionlocator + "option");
		}
	}
	...
	public void mouseHoverJScript(By locator, String Choice, int waitTime) {
		try {
			...
			log.error("Element with " + locator + " to be hovered on " + Choice
					+ " is not attached to the page document - StaleElementReferenceException");
			Assert.fail("Element with " + locator + " to be hovered on " + Choice
					+ " is not attached to the page document - StaleElementReferenceException");
		} catch (NoSuchElementException e) {
			log.error("Element with " + locator + " to be hovered on " + Choice + " was not found in DOM in time - "
					+ waitTime + " - NoSuchElementException");
			Assert.fail("Element with " + locator + " to be hovered on " + Choice + " was not found in DOM in time - "
					+ waitTime + " - NoSuchElementException");
		} catch (Exception e) {
		...
		try {
			if (isElementPresent(Sourcelocator, waitTime)) {
				...
				if (isElementPresent(Destinationlocator, waitTime)) {
					...
					Assert.fail(
							"Destination Element with locator " + Destinationlocator + " was not displayed to drop");
				}
			} else {
			...
			log.error("Element " + Sourcelocator + "or" + Destinationlocator + " was not found in DOM in time - "
					+ waitTime + " - NoSuchElementException");
			Assert.fail("Element " + Sourcelocator + "or" + Destinationlocator + " was not found in DOM in time - "
					+ waitTime + " - NoSuchElementException");
		} catch (Exception e) {
	...
	public void safeSelectContextMenuOption(By locator, int iOptionIndex, int waitTime) {
		try {
			if (isElementPresent(locator, waitTime)) {
				...
				log.error("Element with locator " + locator
						+ "is not displayed to perform content menu operation by index: " + iOptionIndex);
				Assert.fail("Element with locator " + locator
						+ "is not displayed to perform content menu operation by index: " + iOptionIndex);
			}
		} catch (StaleElementReferenceException e) {
			log.error("Context menu option by index: " + iOptionIndex + " for Element with " + locator
					+ " is not attached to the page document");
			Assert.fail("Context menu option by index: " + iOptionIndex + " for Element with " + locator
					+ " is not attached to the page document");
		} catch (NoSuchElementException e) {
			log.error("Context menu option by index: " + iOptionIndex + " for Element with " + locator
					+ " was not found in DOM");
			...
			log.error("Unable to select context menu option by index: " + iOptionIndex + " for Element with" + locator
					+ " - " + e);
			Assert.fail(
					"Unable to select context menu option by index: " + iOptionIndex + " for Element with" + locator);
		}
	}
	...
	public void safeTypeUsingRobot(By locator, String text, String caseType, int... optionWaitTime) {
		try {
			...
			Assert.fail(
					"Text " + text + " to be entered in field " + locator + " is not attached to the page document");
		} catch (NoSuchElementException e) {
		...
		try {
			if (isElementPresent(slocator, waitTime)) {
				...
				if (isElementClickable(slocator, waitTime)) {
					...
					robot.keyRelease(KeyEvent.VK_ENTER);
					Thread.sleep(5000);
					log.info("File at location " + sFileLocation + " is Uploaded by upload field " + slocator
	...
	public void safeJavaScriptClick(WebElement locator, int waitTime) {
		try {
			log.info("Clicking on element with " + locator + " using java script click");
			// waitUntilClickable(locator,waitTime);
	...
	public void safeJavaScriptType(By locator, String sText, int waitTime) throws Exception {
		try {
			...
			log.error("Text " + sText + " to be entered in field with " + locator
					+ " is not attached to the page document - StaleElementReferenceException");
			Assert.fail("Text " + sText + " to be entered in field with " + locator
					+ " is not attached to the page document - StaleElementReferenceException");
		} catch (NoSuchElementException e) {
			log.error("Text " + sText + " to be entered in field with " + locator
					+ " was not found in DOM in time - NoSuchElementException");
			Assert.fail("Text " + sText + " to be entered in field with " + locator
					+ " was not found in DOM in time - NoSuchElementException");
		} catch (Exception e) {
	...
	public void safeJavaScriptClearType(By locator, String sText, int waitTime) {
		try {
			...
			log.error("Text " + sText + " to be entered after " + " clear the field with " + locator
					+ " is not attached to the page document - StaleElementReferenceException");
			Assert.fail("Text " + sText + " to be entered after " + " clear the field with " + locator
					+ " is not attached to the page document - StaleElementReferenceException");
		} catch (NoSuchElementException e) {
			log.error("Text " + sText + " to be entered after " + " clear the field with " + locator
					+ " was not found in DOM in time - " + waitTime + " - NoSuchElementException");
	...
	public String safeGetAttribute(By locator, String sAttributeValue, int waitTime) {
		...
		try {
			...
			log.error(
					"Element " + locator + " was not found in DOM in time - " + waitTime + " - NoSuchElementException");
			Assert.fail(
					"Element " + locator + " was not found in DOMin time - " + waitTime + " - NoSuchElementException");
		} catch (Exception e) {
	...
	public String safeJavaScriptGetAttribute(By locator, String sAttributeValue, int waitTime) {
		...
		try {
			...
			log.error("To get attribute value of type " + sAttributeValue + " for Element with " + locator
					+ " is not attached to the page document - StaleElementReferenceException");
			Assert.fail("To get attribute value of type " + sAttributeValue + " for Element with " + locator
					+ " is not attached to the page document - StaleElementReferenceException");
		} catch (NoSuchElementException e) {
			log.error("To get attribute value of type " + sAttributeValue + " for Element with " + locator
					+ " was not found in DOM in time - " + waitTime + " - NoSuchElementException");
	...
	public boolean safeVerifyListBoxValue(By locator, String value, int sWaitTime) throws Exception {
		...
		try {
			...
			log.error(
					"Listbox item: " + value + " in listbox with" + locator + " is not attached to the page document");
			Assert.fail(
					"Listbox item: " + value + " in listbox with" + locator + " is not attached to the page document");
		...
		try {
			if (isElementPresent(locator, getWaitTime(waitTime))) {
				...
				if (!(sTemp.contains(expected) ^ contains)) {
					bvalue = true;
					log.info("Got attribute value " + sTemp + " of type " + attributeName + " from the element "
							+ locator + " and it is matched with expected " + expected);
				}
			} else {
				log.error("Element " + locator + " was not found in DOM");
				Assert.fail("Element " + locator + " was not found in DOM");
	...
	public String getAttributeValue(By locator, String attributeName, int... waitTime) {
		...
		try {
			...
			log.error("To get attribute value of type " + attributeName + " for Element with " + locator
					+ " is not attached to the page document");
			Assert.fail("To get attribute value of type " + attributeName + " for Element with " + locator
					+ " is not attached to the page document");
		} catch (NoSuchElementException e) {
			log.error("To get attribute value of type " + attributeName + " for Element with " + locator
					+ " was not found in DOM");
	...
	// if(sys.getProperty("HighlightElements").equalsIgnoreCase("true"))
	// {
	// String attributevalue="border:3px solid green;";//change border width and
	// color values if required
	// JavascriptExecutor executor= (JavascriptExecutor) driver;
	// String getattrib=element.getAttribute("style");
	...
	public void switchToWindow(String title) {
		try {
			...
			if (numWindows >= 2) {
				...
				if (!bFlag) {
					log.error("Window with sepcified titlt " + title
							+ " doesn't exists. Please check the window title or wait until the new window appears");
					Assert.fail("Window with sepcified number  " + title
							+ "doesn't exists. Please check the window title or wait until the new window appears");
				}
			} else {
	 ...
	 * locater
	 * 
	 * @param locater, unique element locater @param, waitTime
	 *
	 **/
	...
	public void switchToWindow(By locator, int... waitTime) {
		try {
			...
			if (numWindows >= 2) {
				...
				if (!bFlag) {
					log.error("Window with sepcified locator " + locator
							+ " doesn't exists. Please check the locator or wait until the new window appears");
					Assert.fail("Window with sepcified locator  " + locator
							+ "doesn't exists. Please check the locator or wait until the new window appears");
				}
			} else {
				log.info("Can not switch to new window as there is only one window, wait until the new window appears");
	...
	/**
	 * This method waits for Specified URL to load and writes page load time to load
	 * URL to 'PageLoadTime_Summary.html' file.
	 * 
	 ...
	 * @return void (nothing)
	 */
	public void setVerificationHighlight(By by) {
		WebElement element = driver.findElement(by);
		if (sys.getProperty("HighlightElements").equalsIgnoreCase("true")) {
			String attributevalue = "border:4px solid #cccc00;";
			JavascriptExecutor executor = (JavascriptExecutor) driver;
			String getattrib = element.getAttribute("style");
			executor.executeScript("arguments[0].setAttribute('style', arguments[1]);", element, attributevalue);
			try {
	 ...
	 * @return void (nothing)
	 */
	public void setVerificationHighlight(By by, String color) {
		WebElement element = driver.findElement(by);
		if (sys.getProperty("HighlightElements").equalsIgnoreCase("true")) {
			String attributevalue = "border:4px solid " + color;// #cccc00;";
			JavascriptExecutor executor = (JavascriptExecutor) driver;
			String getattrib = element.getAttribute("style");
			executor.executeScript("arguments[0].setAttribute('style', arguments[1]);", element, attributevalue);
			try {
